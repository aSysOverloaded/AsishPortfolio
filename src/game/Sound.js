/* ==========================================================================
   PROGRAMMATIC WEB AUDIO SYSTEM: CAPTAIN CLAW SOUND SYSTEM
   ========================================================================== */
...
class SoundSystem {
  constructor() {
    ...
    this.shantyInterval = null;
    this.masterVolume = null;
    this.musicVolume = null;
    this.sfxVolume = null;

    // Simple nautical sea-shanty score (note pitches in Hz, durations in seconds)
    // Inspired by classic sea shanties (e.g., Leave Her Johnny / Drunken Sailor)
    this.shantyMelody = [
      // Phrase 1
    ...
    // Frequencies mapping for standard retro 8-bit octaves
    this.notesFreqs = {
      "rest": 0,
      "A3": 220.00, "B3": 246.94,
  ...
  // Initialize audio context safely upon interaction
  init() {
    if (this.ctx) return;
    ...
    // Setup gain nodes for clean volume leveling
  ...
  // Synthesizes individual custom notes
  playOscillator(freq, type, duration, gainNode, attack = 0.02, decay = 0.1) {
    if (!this.ctx) return;
    this.resumeContext();
  ...
  playCoin() {
    ...
    osc2.frequency.setValueAtTime(987.77 * 1.5, now); // soft harmonic
  ...
  // 4. Synthesize Hurt / Fall sound (Descending harsh tone)
  playHurt() {
    if (!this.sfxEnabled || !this.ctx) return;
    this.resumeContext();
  ...
  // 5. Looping Background sea-shanty melody
  playShantyLoop() {
    ...
    const playNextNote = () => {
      ...
      if (freq > 0) {
        // Synthesizing using an 8-bit retro triangle wave (clean woodwind feel)
        this.playOscillator(freq, "triangle", dur - 0.05, this.musicVolume, 0.01, 0.08);
        
        // Add a very soft backing rhythm oscillator (square wave playing 1 octave lower)
        const rootNoteName = item.note.charAt(0) + (parseInt(item.note.charAt(1)) - 1);
        const backingFreq = this.notesFreqs[rootNoteName];
        if (backingFreq > 0) {
          this.playOscillator(backingFreq, "square", dur - 0.1, this.musicVolume, 0.02, 0.15);
        }
      }

      this.melodyIndex = (this.melodyIndex + 1) % this.shantyMelody.length;
      
      // Schedule the next note event based on the current note duration
      this.shantyInterval = setTimeout(playNextNote, dur * 1000);
    };
  ...
  stopShantyLoop() {
    if (this.shantyInterval) {
      clearTimeout(this.shantyInterval);
      this.shantyInterval = null;
    }
  }
